...
2. **Nsight profiling**: run nsys to verify compute/transfer overlap is actually happening (not just added latency).

3. **cuBLAS vs MMQ benchmark**: determine optimal compute path for Kepler (GGML_CUDA_FORCE_CUBLAS=ON vs MMQ).

### Medium Priority
4. **Prefetch buffer sizing**: determine optimal pinned host memory for staging buffers (limited by 128GB system RAM).

5. **NUMA replication revisit**: original implementation was broken (2.3x slower). Consider pointer-swap approach per roblee04/numa_llamacpp (proven 1.4-1.75x on dual EPYC).

6. **K80 intra-card pipeline optimization**: exploit PIX connection for true chip-to-chip pipelining within each K80 card.

### Phase 2 (Qwen3.8/Qwen4 Optimization)
7. Verify NCCL stagger is active: add runtime logging to confirm staggered path is taken vs fallback standard NCCL.
